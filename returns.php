...
function addReturnBottleRow() {
  ...
  const html = `
    <div class="return-bottle-row" id="${rowId}" style="display:flex;gap:10px;align-items:flex-end;margin-bottom:12px;padding:12px;background:white;border-radius:6px;border:1px solid #eee;">
      ...
      <input type="number" min="0" placeholder="Cases" class="case-quantity-input" style="display:none;width:80px;padding:8px;border:1px solid #ddd;border-radius:4px;font-size:13px;" value="0">
...
function addReturnBottleRowWithData(bottle) {
  ...
  const html = `
    <div class="return-bottle-row" id="${rowId}" style="display:flex;gap:10px;align-items:flex-end;margin-bottom:12px;padding:12px;background:white;border-radius:6px;border:1px solid #eee;">
      ...
      <input type="number" min="0" placeholder="Cases" class="case-quantity-input" style="${bottle.with_case ? '' : 'display:none;'}width:80px;padding:8px;border:1px solid #ddd;border-radius:4px;font-size:13px;" value="${bottle.case_quantity || 0}">
...
function updateReturnBottleInfo(select) {
  ...
  if (availableBottlesData[key]) {
    ...
    if (wc && info.with_case) { wc.checked = true; if (cas) { cas.value = info.case_quantity || 0; cas.style.display = ''; } }
    else if (wc) { wc.checked = false; if (cas) { cas.value = 0; cas.style.display = 'none'; } }
  }
}
  ...
  if (wc) {
    wc.addEventListener('change', function() {
      if (this.checked) {
        ...
        if (cas) { cas.style.display = 'none'; cas.value = 0; }
      }
    });
  }
  if (qty) {
