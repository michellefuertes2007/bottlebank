  ...
  if (empty($customer_name)) {
    ...
    if (!$bottles || !is_array($bottles) || count($bottles) === 0) {
      ...
      if (!$all_valid) {
        ...
        $ins = $conn->prepare("INSERT INTO deposit (user_id, customer_name, bottle_type, quantity, with_case, case_quantity, amount, deposit_date) VALUES (?, ?, ?, ?, ?, ?, ?, NOW())");
        $ins->bind_param('issiiid', $user_id, $customer_name, $summary_type, $summary_qty, $aggregate_with_case, $aggregate_case_quantity, $total_amount);
        ...
        if ($ins->execute()) {
          ...
          $log->bind_param('issidsii', $user_id, $customer_name, $summary_type, $summary_qty, $total_amount, $stock_details, $aggregate_with_case, $aggregate_case_quantity);
          $log->execute();
          $log->close();
          ...
          foreach ($deposit_summary as $ds) {
            ...
            $plog = $conn->prepare("INSERT INTO stock_log (user_id, action_type, customer_name, bottle_type, quantity, amount, details, with_case, case_quantity, bottle_size) VALUES (?, 'Deposit', ?, ?, ?, ?, ?, ?, ?, ?)");
            $plog_with_case = intval($ds['with_case']);
            $plog_case_quantity = intval($ds['case_quantity']);
            $plog->bind_param('issidsiis', $user_id, $customer_name, $ds['bottle_type'], $ds['quantity'], $per_amount, $per_details, $plog_with_case, $plog_case_quantity, $ds['bottle_size']);
            $plog->execute();
            $plog->close();
          }
  ...
  const rowId = 'bottle_' + (++bottleCount);
    ...
    <div class="bottle-row" id="${rowId}" style="display:flex;gap:10px;align-items:flex-end;margin-bottom:12px;padding:12px;background:white;border-radius:6px;border:1px solid #eee;">
      <select onchange="updateBottleInfo(this)" style="flex:1.2;padding:8px;border:1px solid #ddd;border-radius:4px;font-size:13px;">
        ...
        ${bottleTypes.map(b => `<option value="${b.type_id}" data-type-name="${b.type_name}" data-bottle-size="${b.bottle_size}" data-price="${b.price_per_bottle}">${b.type_name} ${b.bottle_size} ${b.price_per_bottle ? '₱'+b.price_per_bottle.toFixed(2): ''}</option>`).join('')}
      </select>
